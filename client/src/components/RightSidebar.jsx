      ...
      <div className='px-5 text-xs'>
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {msgImages.map((url, index) => (
            <div key={index} onClick={() => window.open(url, '_blank', 'noopener,noreferrer')} className='cursor-pointer rounded'>
              <img src={url} alt="media item" className='h-full rounded-md' />
            </div>
          ))}
        </div>
      </div>
